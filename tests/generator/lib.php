...
class local_artqtml_generator extends component_generator_base {
    ...
    public function create_question(array $record): \stdClass {
        ...
        $questionbankid = null;
        if ($movedout && !empty($record['movecourse'])) {
            $courseid = (int) $DB->get_field('course', 'id', ['shortname' => $record['movecourse']], MUST_EXIST);
            $category = $this->create_move_target_category($courseid);
            /** @var \core_question_generator $questiongenerator */
            $questiongenerator = $this->datagenerator->get_plugin_generator('core_question');
            $moodleq = $questiongenerator->create_question('truefalse', null, [
                'category' => $category->id,
                'name'     => $questioncode,
            ]);
            $questionbankid = (int) $moodleq->id;
        } else if ($createbank && !empty($generation->draftcategoryid)) {
        ...
        $approved = (int) ($record['approved'] ?? ($movedout ? 1 : 0));
